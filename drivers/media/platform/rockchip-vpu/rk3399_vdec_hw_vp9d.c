...
static void rk3399_vdec_vp9d_config_registers(struct rockchip_vpu_ctx *ctx)
{
	...
	reg = RKVDEC_VP9_TX_MODE(entropy->current_entropy_ctx.tx_mode)
		| RKVDEC_VP9_FRAME_REF_MODE(entropy->current_entropy_ctx.reference_mode);
...
static void adapt_probs(struct rockchip_vpu_ctx *ctx, u8 *count_info)
{
	...
	if (ctx->run.vp9d.entropy->current_entropy_ctx.reference_mode != PRED_COMPREF) {
		for (i = 0; i < 5; i++) {
	...
	/* tx size */
	if (ctx->run.vp9d.entropy->current_entropy_ctx.tx_mode == TX_SWITCHABLE) {
		for (i = 0; i < 2; i++) {
	...
	if (frmhdr->interp_filter == FILTER_SWITCHABLE) {
		for (i = 0; i < 4; i++) {
